$(document).ready(function () {
    ...
    const useDefaultAddressCheckbox = $('#use-default-address');
    const manualBillingAddressFields = $('#manual-billing-address-fields');
    const defaultAddressDisplay = $('#default-address-display');
    ...
    /**
     * Toggles the visibility and enabled state of manual billing address fields
     * based on the "Use default address" checkbox and user authentication status.
     */
    function toggleBillingAddressFields() {

    if (useDefaultAddressCheckbox.length) { 
        if (useDefaultAddressCheckbox.prop('checked')) {
            manualBillingAddressFields.css('display', 'none');
            if (defaultAddressDisplay.length) defaultAddressDisplay.css('display', 'block');
            
            manualBillingAddressFields.find('[required]').each(function() {
                $(this).attr('data-original-required', 'true').removeAttr('required');
            });
            manualBillingAddressFields.find('input, select').attr('disabled', true);

        } else {
            manualBillingAddressFields.css('display', 'block');
            if (defaultAddressDisplay.length) defaultAddressDisplay.css('display', 'none');
            
            manualBillingAddressFields.find('[data-original-required]').each(function() {
                $(this).attr('required', true);
            });
            
            // Re-enable fields ONLY if card payment is selected
    ...
    // Set initial state on load based on whether user is authenticated and if default address was checked
    toggleBillingAddressFields();
    useDefaultAddressCheckbox.on('change', toggleBillingAddressFields);
