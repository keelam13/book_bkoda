...
def _calculate_reschedule_financials(original_booking, new_trip, policy):
    ...
    time_until_departure = \
        original_booking.original_departure_time - timezone.now()
    ...
    elif time_until_original_departure >= \
            policy.late_rescheduling_cutoff_hours:
        reschedule_type_message = "Late Reschedule"
        rescheduling_charge = \
            original_total_price * policy.late_rescheduling_charge_percentage
...
def _calculate_cancellation_financials(booking, policy):
    ...
    if booking.is_rescheduled and booking.original_departure_time:
        ...
        departure_datetime = timezone.make_aware(
            datetime.combine(booking.trip.date, booking.trip.departure_time))

    current_time = timezone.now()
    time_until_departure_hours = (
        departure_datetime - current_time).total_seconds() / 3600
    ...
    if time_until_departure_hours > policy.free_cancellation_cutoff_hours:
        ...
        refund_type_message = \
            f"{int((1 - cancellation_fee_rate) * 100)}%"
        f"(due to late cancellation fee)"
    else:
        refund_amount = Decimal('0.00')  # No refund
        refund_type_message = \
            f"NONE (less than {policy.late_cancellation_cutoff_hours}"
...
def _create_new_rescheduled_booking(
        request, original_booking, new_trip, new_booking_params):
    """
    ...
    original_booking.trip.available_seats += \
...
def _process_refund(request, booking, refund_amount, metadata):
    ...
    STRIPE_MOCK_REFUNDS is used to simulate refunds in development/testing
    environments.
    """
        ...
        # Added stripe refund simulation logic for future development
            ...
            try:
                ...
                if refund.status == 'succeeded':
                    ...
                    messages.success(
                        request,
                        f"Refund of Php{refund_amount:.2f}"
                        f"processed successfully via Stripe.")
                else:
                    booking.refund_status = 'PENDING'
                    messages.warning(
                        request,
                        f"Stripe refund status: {refund.status}."
                        f"It may still be processing or require review."
                        f"We will inform you once it's complete.")
            except stripe.error.StripeError as e:
                booking.refund_status = 'FAILED'
                booking.save()
                raise Exception(
                    f"A Stripe error occurred during refund processing: {e}")
    else:
        booking.refund_status = 'PENDING'
        booking.refund_amount = refund_amount
        send_booking_email(booking, email_type='refund_processing')
